...
class ObjLocaliser(object):
    ...
    def wrapping(self):
        ...
        #Leo aqui
        if im2.shape[0]!=0 and im2.shape[1]!=0:
            resized = cv.resize(im2,(DIMENSION,DIMENSION))
            return resized

        #tem algumas imagem cujo shape sao 0-0-3, fiz o retorno abaixo parar esses casos, não sei o que isso pode implicar.
        else:
            pass
    ...
    def intersectionOverUnion(self, boxA, boxB):
        ...
        #interArea = np.array(interArea, dtype=numpy.float64)
        interArea = np.int64(max(0, xB - xA + 1) * max(0, yB - yA + 1))
        interArea = np.dtype(np.int64).type(interArea)
    ...
    def my_draw(self):
        ...
        #ax.imshow(self.image_playground,cmap='gray')
        ax.imshow(self.image_playground)
